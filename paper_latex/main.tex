...
\author{Arushi~Vaidya,~Aryan~Gupta,~Ananya~S.~Hebbar,~and~Manonmani~S.%
\IEEEcompsocitemizethanks{
\IEEEcompsocthanksitem A. Vaidya, A. Gupta, and A. S. Hebbar are with the
Department of Computer Science and Engineering, RV College of Engineering,
Bengaluru 560059, India.
E-mail: \{arushivaidya.cs23, aryangupta.cs23, ananyashebbar.cs23\}@rvce.edu.in
...
\begin{tikzpicture}[
  ...
  box/.style={
    draw, rounded corners=4pt, minimum height=0.75cm,
    text centered, text width=2.0cm, line width=0.8pt
  },
  ...
  storebox/.style={box, fill=colorE!10, draw=colorE!50, dashed},
  arrow/.style={-{Stealth[length=5pt]}, line width=0.9pt, color=arrowcol},
  dasharrow/.style={-{Stealth[length=5pt]}, dashed, line width=0.7pt, color=gray!70},
  groupbox/.style={draw=gray!40, rounded corners=5pt, fill=gray!6,
                   inner sep=6pt, line width=0.6pt}
]

%% ── Row 0: Input ─────────────────────────────────────────────
\node[sharedbox, text width=2.2cm] (audio) {Audio\\Recording};
\node[sharedbox, right=0.55cm of audio, text width=2.2cm] (asr)
      {ASR Module\\(Whisper / Google)};
\node[sharedbox, right=0.55cm of asr, text width=2.2cm] (preproc)
      {Transcript\\Pre-processing};

%% ── Row 1: Training path (top) ───────────────────────────────
\node[trainbox, above right=0.55cm and 0.55cm of preproc, text width=2.4cm]
      (gemini) {Gemini\\Annotation\\Engine};
\node[trainbox, right=0.55cm of gemini, text width=2.4cm]
      (valida) {Position\\Validation\\(Alg.~\ref{alg:validation})};
\node[trainbox, right=0.55cm of valida, text width=2.4cm]
      (train)  {spaCy NER\\Training\\(30 epochs)};
\node[storebox, right=0.55cm of train, text width=2.0cm]
      (model)  {Trained\\NER Model};

%% ── Row 2: Inference path (bottom) ───────────────────────────
\node[inferbox, below right=0.55cm and 0.55cm of preproc, text width=2.4cm]
      (ner)    {spaCy NER\\Inference\\(12\,ms)};
\node[inferbox, right=0.55cm of ner, text width=2.4cm]
      (struct) {Structured\\Medical\\Extraction};
\node[inferbox, right=0.55cm of struct, text width=2.4cm]
      (fhir)   {FHIR\,R4\\Resource\\Mapping};
\node[inferbox, right=0.55cm of fhir, text width=2.0cm]
      (ehr)    {EHR\\Integration};

%% ── Arrows: shared input ──────────────────────────────────────
\draw[arrow] (audio)   -- (asr);
\draw[arrow] (asr)     -- (preproc);
\draw[arrow] (preproc) -- ++(0,1.0) -| (gemini);
\draw[arrow] (preproc) -- ++(0,-1.0) -| (ner);

%% ── Arrows: training path ─────────────────────────────────────
\draw[arrow] (gemini)  -- (valida);
\draw[arrow] (valida)  -- (train);
\draw[arrow] (train)   -- (model);
\draw[dasharrow] (model) -- ++(0,-0.9) -| (ner)
      node[midway, below, font=\scriptsize, color=colorE!80] {deploy};
...
%% ── Bounding group boxes ─────────────────────────────────────
\begin{scope}[on background layer]
  \node[groupbox, fit=(gemini)(valida)(train)(model),
        label={[font=\scriptsize\bfseries, color=colorA]above:Training Mode (Offline)}]
        (traingroup) {};
  \node[groupbox, fit=(ner)(struct)(fhir)(ehr),
        label={[font=\scriptsize\bfseries, color=colorB]below:Inference Mode (Real-Time)}]
        (infergroup) {};
  \node[groupbox, fit=(audio)(asr)(preproc),
        fill=colorD!5,
        label={[font=\scriptsize\bfseries, color=colorD]above:Shared Input Stage}]
        (sharedgroup) {};
\end{scope}

\end{tikzpicture}
\caption{ClinicalMind end-to-end pipeline architecture. The shared input
stage (amber) converts audio to pre-processed text. In \emph{training mode}
(blue), the Gemini annotation engine generates span-annotated data that trains
the spaCy NER model. In \emph{inference mode} (green), the deployed model
processes new conversations at 12\,ms per conversation, producing FHIR\,R4
resources for EHR integration—with no LLM dependency at runtime.}
